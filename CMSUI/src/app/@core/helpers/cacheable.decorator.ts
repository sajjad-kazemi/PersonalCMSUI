...
export function Cacheable(
  cacheKey: string,
  options?: { ttl?: number; maxSize?: number; handleError?: boolean },
) {
  return function (
    target: any,
    propertyKey: string,
    ...
    descriptor.value = function (...args: any[]) {
      ...
      if (
        cacheEntry &&
        (!options?.ttl || now - cacheEntry.timestamp < options.ttl)
      ) {
      ...
      const resultObservable: Observable<any> = originalMethod
        .apply(this, args)
        .pipe(
          tap((result) => {
